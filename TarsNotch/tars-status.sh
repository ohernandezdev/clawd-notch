#!/bin/bash
# Tars Notch status hook — writes Claude Code session state to a per-user temp directory
...
if hook in ('SessionStart', 'session_start'):
    ...
    tars_tars_notif_dir = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'tars-sessions')
    notif_path = os.path.join(tars_tars_notif_dir, '_notify_' + sid + '.json')
    tmpfd2, tmppath2 = tempfile.mkstemp(dir=tars_tars_notif_dir, suffix='.tmp')
    try:
...
outpath = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'tars-sessions', sid + '.json')
...
tmpfd, tmppath = tempfile.mkstemp(dir=os.path.join(os.environ.get('TMPDIR', '/tmp'), 'tars-sessions'), suffix='.tmp')
try:
...
# Notifications are handled by the Tars Notch app
" 2>/dev/null
