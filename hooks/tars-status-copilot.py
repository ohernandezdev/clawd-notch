...
if hook == 'Stop':
    ...
    tars_notif_dir = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'tars-sessions')
    notif_path = os.path.join(tars_notif_dir, '_notify_' + sid + '.json')
    tmpfd2, tmppath2 = tempfile.mkstemp(dir=tars_notif_dir, suffix='.tmp')
    try:
